...
void start_team(Config *cfg , FurnitureSet *furniture , int round , int team_id)
{
    ...
    for (int i = 0 ; i < cfg->num_workers - 1 ; i++)
    {
        //After forking all workers, parent closes ALL pipe ends:
        close(pipes.forward[i][0]) ;
        close(pipes.forward[i][1]) ;
        close(pipes.backward[i][0]) ;
        close(pipes.backward[i][1]) ;
    }
    ...
    free_pipes(&pipes) ;  //(as a precaution,) Clean up pipes after all workers have finished to free allocated resources and prevent memory leaks, ensuring proper cleanup of the pipes used for inter-process communication in the team
}