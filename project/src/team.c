...
void start_team(Config *cfg , FurnitureSet *furniture , int round)
{
    ...
    for (int i = 0 ; i < cfg->num_workers ; i++)
    {
        ...
        {
            worker_process(i , &pipes , cfg , furniture , round) ;  //Call the worker_process function with the worker ID, pipes, configuration, furniture set, and current round to start the worker's execution and handle the assembly line processing for the assigned pieces
            exit(0) ;  //safety exit
        }
    }
