...
int main()
{
    ...
    print_sample("Team A" , set.teamA , set.count , 10) ; 
    print_sample("Team B" , set.teamB , set.count , 10) ;
    print_sample("Sorted" , set.sorted , set.count , 10) ;
    ...
    for (int round = 1; round <= cfg.rounds_to_win; round++)
    {
        ...
        if (init_furniture_set(&set , cfg.num_pieces , mode , min , max) != 0)
        {
            printf("Error initializing furniture\n");
            exit(1);
        }

        start_team(&cfg, &set, round);

        free_furniture_set(&set);
    }
