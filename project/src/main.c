...
int main()
{
    ...
    // ------------------------
    printf("=====================================\n") ;
    printf(" Hi! Welcome to Furnishing Simulator \n") ;
    printf("=====================================\n") ;
    ...
    // ------------------------------------------------------
    // Final message before program termination
    // ------------------------------------------------------
    ...
    print_sample("Team A" , set.teamA , set.countA , 10) ; 
    print_sample("Team B" , set.teamB , set.countB , 10) ;
    print_sample("Sorted" , set.sorted , cfg.num_pieces , 10) ;
    ...
    for (int round = 1; round <= cfg.rounds_to_win; round++)
    {
        reset_signals() ;  //Reset signal flags before starting a new round to ensure accurate tracking of team completion status for each round without interference from previous rounds

        printf("\n=== ROUND %d ===\n" , round) ;  //Print round number for clarity

        FurnitureSet set ;  //Declare a new FurnitureSet for each round to ensure fresh pieces are generated for each round of the competition

        if (init_furniture_set(&set , cfg.num_pieces , mode , min , max) != 0)  //Initialize the furniture set for the current round based on user configuration and generation mode, check for errors
        {
            printf("Error initializing furniture\n") ;
            exit(1) ;  //Exit with error if furniture initialization fails to ensure proper error handling and prevent undefined behavior
        }

        start_team(&cfg , &set , round , 0) ;  //Start team (ID = 0) with the current configuration, furniture set, and round number to run the assembly line simulation for that team and round

        start_team(&cfg , &set , round , 1) ;  //Start team (ID = 1) with the current configuration, furniture set, and round number to run the assembly line simulation for that team and round

        free_furniture_set(&set) ; //Free allocated memory for the furniture set after the round is completed to prevent memory leaks and ensure proper cleanup of resources used for the furniture pieces in that round
    }
