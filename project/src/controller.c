...
void run_controller(Config *cfg , int mode , int min , int max)
{
    ...
    for (round = 1; round <= target; round++)
    {
        ...
        while (!team0_done && !team1_done)  //Loop to wait for signals from both teams indicating their completion status, allowing the controller to pause and wait for the outcome of the round before proceeding to determine the winner based on which team sent the signal first, and ensuring that the controller does not proceed prematurely before receiving the necessary signals from the worker processes in each team
            pause() ;  //Pause the controller process until it receives a signal from either Team 0 or Team 1 indicating that they have completed their tasks, allowing the controller to wait for the completion of both teams and determine the winner of the round based on the signals received from the worker processes in each team
