...
void worker_process(int id , Pipes *pipes , Config *cfg , FurnitureSet *furniture , int round)
{
    ...
    if (id == 0)
    {
        ...
        while (furniture->count > 0)  //Continue picking pieces until all pieces have been successfully sent and accepted
        {
            int piece = pick_random_piece(furniture , last_failed) ;  //Pick a random piece from the furniture set, avoiding the last failed piece to increase chances of acceptance

            printf("[SOURCE] sending %d\n" , piece) ;  //Log the piece being sent for debugging and tracking purposes
            ...
            if (result == ACCEPT)
            {
                printf("[SOURCE] accepted %d\n" , piece) ;  //Log acceptance for debugging and tracking purposes
                remove_piece(furniture , piece) ;  //Remove the accepted piece from the furniture set to prevent it from being picked again
                last_failed = -1 ;  //Reset last failed piece since the current piece was accepted
            }
            else
            {
                printf("[SOURCE] rejected %d by the sink so I put it back\n" , piece) ;  //Log rejection for debugging and tracking purposes
    ...
    else if (id == cfg->num_workers - 1)
    {
        ...
        while (1)
        {
            ...
            printf("[SINK] got %d | expected %d\n" , piece , furniture->sorted[index]) ;  //Log the received piece and the expected piece for debugging and tracking purposes

            int result ;

            if (piece == furniture->sorted[index])  //Check if the received piece matches the expected piece in the sorted reference array
            {
                printf("[SINK] ACCEPT\n") ;  //Log acceptance for debugging and tracking purposes
            ...
            {
                printf("[SINK] REJECT\n") ;  //Log rejection for debugging and tracking purposes
    ...
    else
    {
        while (1)
        {
            ...
            printf("[MIDDLE %d] passing forward %d\n" , id, piece) ;  //Log the piece being forwarded for debugging and tracking purposes
            ...
            printf("[MIDDLE %d] passing backward %d\n" , id, piece) ;  //Log the piece being sent back for debugging and tracking purposes
