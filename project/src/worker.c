...
Delay depends on:
1. Base randomness:
   Introduces natural variability between workers.

2. Round-based fatigue:
   Delay increases with round number (global slowdown over time).

3. Piece-based congestion:
   Delay grows linearly with processed pieces (local workload buildup).
*/
static void do_delay(Config *cfg , int round , int *last_delay , int piece_count)
{
    int base = rand() % (cfg->delay_max - cfg->delay_min + 1) + cfg->delay_min ;  //Generate random base delay within the specified range [delay_min , delay_max]
    int fatigue = round * 10 ;  //Calculate fatigue based on the current round (increasing delay as rounds progress)
    int congestion = piece_count * 4 ;  //The more pieces a worker has already processed, the more congested it becomes so it slows down
    
    int delay = base + fatigue + congestion ;  //Total delay is the sum of base delay, fatigue and congestion
...
void worker_process(int id , Pipes *pipes , Config *cfg , FurnitureSet *furniture , int round , int team_id)
{
    ...
    if (id == 0)
    {
        ...
        {
            do_delay(cfg , round , &last_delay , piece_count) ;  //Simulate processing delay with fatigue for the source worker before picking the next piece to send, allowing for a more realistic simulation of the assembly line process and increasing the chances of acceptance by avoiding immediate retries of failed pieces
            piece_count++ ;
    ...
    else if (id == cfg->num_workers - 1)
    {
        ...
        while (!terminate_flag)
        {
            ...
            do_delay(cfg , round , &last_delay , piece_count) ;  //Simulate processing delay with fatigue for the sink worker before validating the piece
            piece_count++ ;
    ...
    else
    {
        ...
        while (!terminate_flag)
        {
            ...
            do_delay(cfg , round , &last_delay , piece_count) ;  //Simulate processing delay with fatigue for middle workers before forwarding the piece to the next worker
            piece_count++ ;
            ...
            do_delay(cfg , round , &last_delay , piece_count) ;  //Simulate processing delay with fatigue for middle workers before sending the result back to the previous worker
            piece_count++ ;
